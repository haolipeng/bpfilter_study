# bpfilter 学习练习

欢迎来到 bpfilter 的实践练习！这个目录包含了一系列渐进式的练习，帮助你从零开始掌握 bpfilter。

## 📚 学习资源

### 规则练习（Rules Exercises）

#### Exercise 1: 基础规则和匹配器 ⭐
**文件：** `exercise_01_basic.bf`
**学习内容：**
- Chain、Rule、Matcher、Verdict 基本概念
- IP 地址匹配（saddr、daddr、snet、dnet）
- 协议匹配（tcp、udp、icmp）
- Counter 和 Log 的基础用法

**预计时间：** 1-2 小时

---

#### Exercise 2: 端口过滤和传输层匹配 ⭐⭐
**文件：** `exercise_02_ports.bf`
**学习内容：**
- TCP/UDP 端口匹配（sport、dport）
- 组合多个匹配器
- 不同 Hook 类型（LOCAL_IN、LOCAL_OUT）
- 实际防火墙场景（SSH 保护、Web 服务器）

**预计时间：** 2-3 小时

---

#### Exercise 3: 使用 Set 进行批量匹配 ⭐⭐⭐
**文件：** `exercise_03_sets.bf`
**学习内容：**
- Set 的定义和使用
- 命名 Set vs 匿名 Set
- IP 黑名单/白名单
- 端口集合管理
- 性能优化技巧

**预计时间：** 2-3 小时

---

#### Exercise 4: 流量统计和监控 ⭐⭐
**文件：** `exercise_04_counters.bf`
**学习内容：**
- Counter 深度使用
- Log 级别（none、meta、transport）
- 流量分析和监控
- 安全事件记录
- 带宽监控技巧

**预计时间：** 2-3 小时

---

### 测试体系文档（Testing Documentation）

#### 📘 [README_testing.md](./README_testing.md) - 测试文档索引
**适合：** 所有想了解测试的开发者
**内容：** 测试文档导航、学习路径、快速开始指南

---

#### 📗 [bpfilter_testing_guide.md](./bpfilter_testing_guide.md) - 完整测试指南
**适合：** 想要深入了解测试体系的开发者
**内容：**
- 测试架构详解
- 单元测试、E2E 测试、CLI 测试
- Mock 系统使用
- 测试最佳实践
- 添加新测试

**预计时间：** 30-45 分钟

---

#### 📙 [testing_quick_reference.md](./testing_quick_reference.md) - 快速参考手册
**适合：** 需要快速查找命令和示例的开发者
**内容：**
- 常用命令速查
- 测试模板
- 调试技巧
- 常见问题解答

**预计时间：** 5-10 分钟

---

#### 📕 [testing_architecture.md](./testing_architecture.md) - 架构图解
**适合：** 想要理解测试系统设计的开发者
**内容：**
- 整体测试架构图
- 执行流程图
- Mock 系统原理
- 测试发现机制

**预计时间：** 15-20 分钟

---

## 🚀 如何使用这些练习

### 1. 准备环境

首先确保已经编译了 bpfilter：

```bash
cd /home/work/bpfilter
cmake -S . -B build -DNO_DOCS=ON -DNO_CHECKS=ON -DNO_BENCHMARKS=ON
make -C build
```

### 2. 启动守护进程

在一个终端窗口中启动守护进程：

```bash
sudo build/output/sbin/bpfilter --transient -v
```

- `--transient`: 测试模式，不持久化配置
- `-v`: 详细日志模式

### 3. 运行练习

在另一个终端窗口中加载练习文件：

```bash
# 加载练习 1
sudo build/output/sbin/bfcli ruleset set --from-file learning_exercises/exercise_01_basic.bf

# 查看加载的规则
sudo build/output/sbin/bfcli ruleset get

# 测试规则...（根据练习文件中的说明）

# 清空规则，准备下一个练习
sudo build/output/sbin/bfcli ruleset flush
```

### 4. 验证规则

每个练习文件都包含了测试方法，例如：

```bash
# 测试 ICMP 规则
ping <your-machine-ip>  # 从其他机器执行

# 查看 counter 是否增加
sudo build/output/sbin/bfcli ruleset get

# 查看日志
sudo journalctl -u bpfilter -f
```

## 📖 学习路径建议

### 第一周：基础掌握
- **Day 1-2:** Exercise 1（基础规则）
- **Day 3-4:** Exercise 2（端口过滤）
- **Day 5:** 复习和实践

### 第二周：进阶技能
- **Day 1-2:** Exercise 3（Set 使用）
- **Day 3-4:** Exercise 4（流量监控）
- **Day 5:** 综合实战项目

### 实战项目建议

完成基础练习后，尝试以下项目：

1. **家庭网关防火墙**
   - 保护内网主机
   - 限制出站流量
   - 统计流量使用

2. **Web 服务器防护**
   - 只开放必要端口（80、443、22）
   - IP 白名单
   - DDoS 基础防护

3. **企业边界防火墙**
   - 分区管理（DMZ、内网、管理网）
   - 服务端口控制
   - 流量审计和日志

## 🔧 常用命令速查

```bash
# 从文件加载规则
sudo build/output/sbin/bfcli ruleset set --from-file <file.bf>

# 从字符串加载规则
sudo build/output/sbin/bfcli ruleset set --from-str "chain test BF_HOOK_NF_LOCAL_IN ACCEPT ..."

# 查看当前规则集
sudo build/output/sbin/bfcli ruleset get

# 清空所有规则
sudo build/output/sbin/bfcli ruleset flush

# 查看守护进程日志
sudo journalctl -u bpfilter -f

# 查看守护进程状态
ps aux | grep bpfilter
```

## 🐛 故障排除

### 问题 1：守护进程启动失败

```bash
# 检查是否已有实例运行
ps aux | grep bpfilter | grep -v grep

# 如果有，先停止
sudo pkill bpfilter

# 重新启动
sudo build/output/sbin/bpfilter --transient
```

### 问题 2：规则加载失败

- 检查规则文件语法
- 查看守护进程日志：`sudo journalctl -u bpfilter -n 50`
- 使用 `-v` 参数获取详细错误信息

### 问题 3：规则不生效

- 确认守护进程正在运行
- 检查 Hook 类型是否正确
- 查看 counter 是否增加（证明规则被匹配）
- 确认默认策略是否符合预期

### 问题 4：无法看到日志

```bash
# 使用 verbose 模式启动守护进程
sudo build/output/sbin/bpfilter --transient -v

# 或指定输出到 syslog
sudo build/output/sbin/bpfilter --transient --syslog

# 查看日志
sudo tail -f /var/log/syslog | grep bpfilter
```

## 📚 进阶学习资源

完成这些练习后，你可以继续学习：

1. **阅读实际示例**
   - `tests/e2e/rulesets/` - 各种 Hook 的完整示例
   - `tests/rules.bpfilter` - 所有功能的综合示例

2. **学习 API 编程**
   - `src/libbpfilter/include/bpfilter/bpfilter.h` - API 定义
   - `src/bfcli/main.c` - CLI 工具实现（API 用法示例）

3. **深入内部实现**
   - `src/bfcli/lexer.l` 和 `parser.y` - 规则解析
   - `src/bpfilter/cgen/` - BPF 代码生成
   - `src/bpfilter/xlate/` - 翻译层实现

4. **运行测试**
   ```bash
   # 运行单元测试
   make -C build unit

   # 运行 E2E 测试
   make -C build e2e

   # 运行集成测试
   make -C build integration
   ```

5. **阅读文档**
   - [doc/usage/bfcli.rst](../doc/usage/bfcli.rst) - 完整命令参考
   - [doc/developers/](../doc/developers/) - 开发者文档
   - https://bpfilter.io/ - 官方文档网站

## 💡 学习技巧

1. **边学边做**：每看到一个新概念，立即用 bfcli 测试
2. **记录笔记**：记录遇到的问题和解决方法
3. **实验变体**：修改示例规则，观察行为变化
4. **阅读日志**：使用 `-v` 和 `log` 理解规则执行过程
5. **参考测试**：`tests/` 目录是最好的学习材料

## 🎯 评估标准

完成每个练习后，确保你能够：

### Exercise 1
- [ ] 独立编写基础规则
- [ ] 理解 Chain 的结构和作用
- [ ] 使用 counter 验证规则
- [ ] 理解不同判决（ACCEPT、DROP）的含义

### Exercise 2
- [ ] 编写端口过滤规则
- [ ] 组合多个匹配器
- [ ] 理解不同 Hook 的区别
- [ ] 实现简单的防火墙配置

### Exercise 3
- [ ] 创建和使用 Set
- [ ] 理解 Set 的性能优势
- [ ] 编写大规模 IP 黑名单
- [ ] 使用 Set 优化规则结构

### Exercise 4
- [ ] 使用 counter 收集流量统计
- [ ] 配置不同级别的 log
- [ ] 分析流量模式
- [ ] 实现监控和告警

## 🤝 获取帮助

如果遇到问题：

1. 查看练习文件中的注释和提示
2. 阅读 [LEARNING_QUICKSTART.md](../LEARNING_QUICKSTART.md)
3. 查看官方文档：https://bpfilter.io/
4. 研究 `tests/` 目录中的示例
5. 查看 GitHub Issues：https://github.com/facebook/bpfilter/issues

## 📝 学习记录

建议创建一个学习日志，记录：

- 完成的练习和日期
- 遇到的问题和解决方法
- 自己编写的规则示例
- 性能测试结果
- 新的发现和想法

示例格式：
```
## 2025-10-31

### 完成
- Exercise 1: 基础规则
- Exercise 2: 端口过滤（50%）

### 学习笔记
- Counter 可以统计包数和字节数
- ICMP 协议值是 1
- 默认策略很重要

### 问题
- Q: 如何匹配端口范围？
- A: 查看文档后发现...

### 下一步
- 完成 Exercise 2
- 尝试编写 SSH 保护规则
```

祝学习愉快！🚀
